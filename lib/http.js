...
class HTTP extends Server {
  ...
  constructor(options) {
    ...
    this.addrindex = this.node.addrindex;
    this.txindex = this.node.txindex;
  ...
  initRouter() {
    ...
    this.post('/account/balance', async (req, res) => {
      ...
      // TODO: pass in hash/index
      const hashes = await this.addrindex.getHashesByAddress(address);

      let balance = 0;
      for (const hash of hashes) {
        const mtx = await this.txindex.getMeta(hash);
        // TODO: filter tx inputs/outputs and update balance
      }

      // TODO: fetch address balance at block hash
      res.json(200,
        {
          block_identifier: {
            index: height,
    ...
    this.post('/block', async (req, res) => {
      ...
      const prevHeight = await this.chain.getHeight(block.prevBlock);
      ...
      res.json(200,
        {
          block: {
            ...
            parent_block_identifier: {
              index: prevHeight,
              hash: block.prevBlock.toString('hex')
            },
