...
class HTTP extends Server {
  ...
  initRouter() {
    ...
    this.post('/network/options', async (req, res) => {
      // TODO: openapi - parse request data
      const options = new NetworkOptions(this.node.pool.options.agent);
    ...
    this.post('/network/status', async (req, res) => {
      ...
      const status = new NetworkStatus(peers,
        this.node.chain.height, this.node.chain.tip, this.network);
